...
const PlanMaster = () => {
    ...
    useEffect(() => {
        if (!fetched) {
            axios
                .get("/api/loan/get-plans/savings")
                .then(function (value) {
    ...
    function validatePlanId() {
        const planExists = schemes.some(plan => plan.schemeCode === formData.schemeCode);
    ...
    const handleSubmit = async () => {
        if(!validatePlanId()) return;
        if (!validate()) return;

        setProgressbar(true);
        try {
            const response = await axios.post(
                "/api/loan/plan-creation/savings",
                {...formData, id: formData.schemeCode},
            );
            if (response.data.success) {
                const filteredSchemes = schemes.filter(({schemeCode}) => schemeCode !== formData.schemeCode);
                setSchemes([...filteredSchemes, formData]);
    ...
    const handleDelete = async (schemeCode) => {
        ...
        try {
            const response = await axios.post("/api/loan/delete-plan/savings", {
                schemeCode,
            });

            if (response.data.success) {
                setSchemes((prev) =>
                    prev.filter((scheme) => scheme.schemeCode !== schemeCode)
                );
        ...
        <>
            ...
            <div className="content">
                ...
                <Card className="shadow p-4 mb-4">
                    ...
                    <CardHeader>
                        <CardTitle tag="h3">Service Charges</CardTitle>
                    </CardHeader>
                    <CardBody>
                        <Row className="mb-3">
                            <Col md="4">
                    ...
                    <CardHeader>
                        <CardTitle tag="h3">SMS Charges</CardTitle>
                    </CardHeader>
                    <CardBody>
                        <Row className="mb-3">
                            <Col md="4">
                    ...
                    <CardFooter className="d-flex justify-content-center gap-2">
                        <Button
                            ...
                            disabled={progressbar || (authStatus.accessLevel?.planCreation !== true && authStatus.role !== 'admin' && authStatus.role !== 'root')}
                        >
                            {progressbar ? (
                                ...
                                <tr key={index}>
                                    ...
                                    <td>
                                        <Button
                                            color="primary"
                                            size="sm"
                                            onClick={() => setFormData({...scheme, updateRequested: true})}
                                        >
                                        ...
                                        {authStatus.accessLevel?.planCreation || authStatus.role === 'admin' || authStatus.role === 'root' ?
                                            <Button
                                                color="danger"
                                                size="sm"
                                                onClick={() => handleDelete(scheme.schemeCode)}
                                            >
