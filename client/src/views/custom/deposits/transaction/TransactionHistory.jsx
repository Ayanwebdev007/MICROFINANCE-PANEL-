...
const TransactionHistory = () => {
  ...
  useEffect(() => {
    const transformedData = transactions.map((tx, index) => {
      ...
      return {
        slNo: index + 1,
        id: tx.id || `DEP${index + 1}`,
        date: tx.date || tx.entryDate || "",
        method: tx.method || "N/A",
        narration: tx.narration || "N/A",
        type: isCredit ? "Deposit" : "Payout",
        amount: amount,
        sign: isCredit ? "+" : "-",
        // balance: tx.balance?.toFixed(2) || "0.00", not covert into number
        balance: isNaN(Number(tx.balance)) ? "0.00" : Number(tx.balance).toFixed(2),
        raw: tx,
      };
    });
  ...
  const onBtnExport = () => {
    if (gridRef.current?.api) {
      gridRef.current.api.exportDataAsCsv({
        fileName: `deposit_transactions_${accountData?.account}.csv`,
