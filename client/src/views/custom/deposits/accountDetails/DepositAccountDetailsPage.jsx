...
const DepositAccountDetailsPage = () => {
  ...
  const handleDownloadCertificate = async () => {
    ...
    if (!accountId || !accountType) {
      setAlert({
        color: "warning",
        message: "Account ID or type missing from URL.",
        display: true,
        timestamp: Date.now(),
      });
      return;
    ...
    try {
      const url = `/api/deposit/${encodeURIComponent(accountType)}/certificate/${encodeURIComponent(accountId)}`;
      ...
      if (contentType.includes('application/json')) {
        ...
        throw new Error(json.error || "Invalid certificate response");
      }

      // Extract filename
      let filename = `certificate_${accountId}.pdf`;
      ...
      const msg =
        err.response?.status === 404 ? "Certificate not issued for this account." :
            ...
            <div>
              ...
              >
                {loadingCertificate ? (
                  ...
                  <>
                    <i className="tim-icons icon-paper me-1" /> Certificate
                  </>
                )}
              </Button>
              <Button
