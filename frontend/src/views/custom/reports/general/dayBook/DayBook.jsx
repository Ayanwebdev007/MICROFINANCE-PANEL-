        ...
        <Card>
          ...
          <CardBody>
            <table className='table table-striped table-bordered'>
              ...
              {dayTransactions.map((transaction)=>{
                if (transaction.creditAmount > 0 && transaction.transactions.length > 0){
                  return <tbody>
                  ...
                  {transaction.transactions.map((pi) => {
                    if (pi.details.type === 'credit'){
                      return <tr key={pi.id}>
                        ...
                        <th className={"text-right"}><b>{(pi.details.method === 'cash' ? 1 : 0) * parseFloat(pi.details.amount)}</b></th>
                        <th className={"text-right"}><b>{(pi.details.method === 'cash' ? 0 : 1) * parseFloat(pi.details.amount)}</b></th>
                        <th className={"text-right"}>{parseFloat(pi.details.amount)}</th>
                      </tr>
                    }
                  })}
                  </tbody>
          ...
          <CardBody>
            <table className='table table-striped table-bordered'>
              ...
              {dayTransactions.map((transaction)=>{
                if (transaction.debitAmount > 0 && transaction.transactions.length > 0){
                  return <tbody>
                  <tr className={'bg-success'}>
                    <th className={'text-center'}>{transaction.glCode}</th>
                    <th className={'text-center'} colSpan={2}>{transaction.glHead}</th>
