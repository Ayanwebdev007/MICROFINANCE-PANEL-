...
function CashAccount(props){
  ...
  const [cstError, setCstError] = React.useState({fromDate: '', toDate: ''});
  ...
  async function onSubmit() {
    ...
    if (checkInput){
      setCstError({fromDate: '', toDate: ''});
      setShowProgress(true);

      try {
        const submit = await axios.post(`/api/reports/general/cash-account-v2`, details);
      ...
      setShowProgress(false);
      notify('please fill all the required fields', 'danger');
    }
  }

  function validateInput(userInput) {
    let valid = true;
    let errorObj = {fromDate: '', toDate: ''};
              ...
              <Card>
                <Row>
                  ...
                  <Col md={'3'}>
                    ...
                    <Button className={"btn-fill mt-3"} color="success" type="button" onClick={onSubmit} disabled={showProgress}>
                      Submit
                    </Button>
                  </Col>
                </Row>
        ...
        <Card>
          <div>
            <div className="form-inline offset-5 mt-2">
              <h3 className="" style={{fontSize:'1.4rem', fontWeight: 'bold'}} id="daybook">Cash Account Report</h3>
          ...
          <CardBody>
            <table className='table table-striped table-bordered'>
              ...
              {dayTransactions.map((transaction)=>{
                if (transaction.creditAmount > 0){
                  ...
                  <tr className={''}>
                    ...
                    <th className={'text-right'}>{parseFloat(transaction.cashCreditAmount || 0).toFixed(2)}</th>
                    <th className={'text-right'}>{(parseFloat(transaction.transferCreditAmount || 0)).toFixed(2)}</th>
                    <th className={'text-right'}>{parseFloat(transaction.creditAmount || '0').toFixed(2)}</th>
                  </tr>
                  </tbody>
                }
              })}
              ...
              <tr className="bg-dark text-white">
                ...
                <th className="text-right " id="totalCashDebit" style={{fontSize:'25px'}}>{grandTotal.cashCreditAmount.toFixed(2)}</th>
                <th className="text-right" id="totalTransferDebit" style={{fontSize:'25px'}}><b>{grandTotal.transferCreditAmount.toFixed(2)}</b></th>
                <th className="text-right" id="totalDebit" style={{fontSize:'25px'}}><b>{grandTotal.creditAmount.toFixed(2)}</b></th>
              </tr>
              </tbody>
            </table>
          </CardBody>
          ...
          <CardBody>
            <table className='table table-striped table-bordered'>
              ...
              {dayTransactions.map((transaction)=>{
                if (transaction.debitAmount > 0){
                  ...
                  <tr className={''}>
                    <th className={'text-center'}>{transaction.glCode}</th>
                    <th className={'text-center'} colSpan={2}>{transaction.glHead}</th>
                    <th className={'text-right'}>{parseFloat(transaction.cashDebitAmount || 0).toFixed(2)}</th>
                    <th className={'text-right'}>{(parseFloat(transaction.transferDebitAmount || 0)).toFixed(2)}</th>
                    <th className={'text-right'}>{parseFloat(transaction.debitAmount || 0).toFixed(2)}</th>
                  </tr>
                  </tbody>
                }
              })}
              ...
              <tr className="bg-dark text-white">
                <th className="text-center" style={{fontSize:'25px', width:'15rem'}}/>
                <th colSpan={2} className="text-center" style={{fontSize:'25px'}}><strong>Grand Total</strong></th>
                <th className="text-right " id="totalCashDebit" style={{fontSize:'25px'}}>{grandTotal.cashDebitAmount.toFixed(2)}</th>
                <th className="text-right" id="totalTransferDebit" style={{fontSize:'25px'}}><b>{grandTotal.transferDebitAmount.toFixed(2)}</b></th>
                <th className="text-right" id="totalDebit" style={{fontSize:'25px'}}><b>{grandTotal.debitAmount.toFixed(2)}</b></th>
              </tr>
              </tbody>
            </table>
          </CardBody>
