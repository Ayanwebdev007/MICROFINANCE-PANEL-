...
const ViewActiveLoans = () => {
    ...
    const handleViewDetails = (rowData) => {
        navigate(`/Active-Group-Loan-Account/Details/${rowData.loanNo}`, {state: rowData});
    };
        ...
        },
        {
            ...
            cellRenderer: (params) => (
                <Button
                    ...
                    onClick={(e) => {
                        e.stopPropagation(); // Prevent row selection
                        handleMemberClick(params.value);
                    }}
                >
                   <span style={{userSelect: 'text', WebkitUserSelect: 'text', MozUserSelect: 'text', msUserSelect: 'text'}}>
                  {params.value} </span>
    ...
    const fetchAllLoans = async (bankId) => {
        ...
        return await axios.post(
            "/api/reports/loan/active-accounts/group-loan",
            payload,
    ...
    const onBtExport = useCallback(() => {
        gridRef.current.api.exportDataAsCsv({
            fileName: `active_group_loans_export_${new Date().toISOString().slice(0, 10)}.csv`,
        });
        notify("Exported active group loans to CSV", "success");
    }, []);
    ...
    async function handleDelete(row) {
        try {
            ...
            const proceed = window.confirm(
                `Are you sure you want to delete loan account: ${row.loanNo}? This action cannot be undone.`
            );
            if (!proceed) return;

            setLoading(true);

            const payload = {loanId: row.loanNo, accountType: 'group-loan'};
            ...
            <div className="content">
                ...
                <Row>
                    <Col md="12">
                        <Card>
                            <CardHeader
                                className="d-flex justify-content-between align-items-center flex-wrap text-white py-3 px-4">
