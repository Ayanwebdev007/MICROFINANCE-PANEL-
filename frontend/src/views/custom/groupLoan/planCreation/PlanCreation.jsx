...
const PlanCreation = () => {
    ...
    // delete group loan plan
    const handleDelete = async (planId) => {
        const confirmDelete = window.confirm("Are you sure you want to delete this plan?");
        if (!confirmDelete) return;

        try {
            const response = await axios.post("/api/loan/delete-plan/group-loan", {
                schemeCode: planId,
            });
    ...
    const handleSubmit = async () => {
        if (validateForm()) {
            setProgressbar(true);
            try {
                const submitData = await axios.post('/api/loan/plan-creation/group-loan', formData);
