...
const GroupForeclosedAccountsList = () => {
  ...
  useEffect(() => {
    const fetchForeclosedAccounts = async () => {
      try {
        const response = await axios.get("/api/loan/foreclosed-accounts/group-loan");
  ...
  const handleDownloadNOC = (accountNumber) => {
    const url = `/api/group-loan/foreclose/noc/${accountNumber}`;
...
export default GroupForeclosedAccountsList;
