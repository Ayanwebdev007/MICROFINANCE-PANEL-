...
const ViewDepositAccounts = () => {
    ...
    const [depositTypeDropDown, setDepositTypeDropDown] = useState([
        {value: "daily-savings", label: "Daily Savings"},
        {value: "recurring-deposit", label: "Recurring Deposit"},
        {value: "thrift-fund", label: "Thrift Fund"},
    ]);
    const [selectedDepositType, setSelectedDepositType] = useState("daily-savings");
    ...
    // 👇 FIXED COLUMN DEFINITIONS — NO DYNAMIC LOGIC
    const colDefs = [
    ...
    const handleDepositTypeChange = async (selectedOption) => {
        const type = selectedOption?.value || "daily-savings";
        setSelectedDepositType(type);
        setRowData([]);
        await fetchDepositAccounts(selectedBank, type); // Always reset to all branches
    }
    ...
    const handleBankSelect = async (selectedOption) => {
        try {
            ...
            await fetchDepositAccounts(bankId, selectedDepositType);
        } catch (e) {
            console.error(e);
            setRowData([]);
    ...
    const init = async () => {
        try {
            ...
            if (bankRes.data.success) {
                setBankDropDown(bankRes.data.data);
            } else {
                            ...
                            <CardHeader
                                ...
                                <div className="d-flex gap-2 mt-2 mt-md-0">
                                    ...
                                    <Button color="secondary" size="sm"
                                            onClick={() => fetchDepositAccounts("", selectedDepositType)}>
                                ...
                                {loading ? (
                                    ...
                                    >
                                        <AgGridReact
                                            ref={gridRef}
                                            rowData={rowData}
                                            columnDefs={colDefs} // 👈 FIXED, NO DYNAMIC LOGIC
