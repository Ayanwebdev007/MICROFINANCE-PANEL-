...
const DepositTransaction = () => {
  ...
  // Handle pre-fill from navigation
  React.useEffect(() => {
    if (location.state?.prefillAccountNo) {
      const accountNumber = location.state.prefillAccountNo;
      setFormData(prev => ({
        ...prev,
        account: accountNumber
      }));
      fetchAccountDetails(accountNumber, formData.accountType);
    }
  }, [location.state]);
  ...
  const fetchAccountDetails = async (accountNumber, accountType) => {
    setAlert({...alert, color: 'info', message: '', display: false,});
    ...
    try {
      const fetchData = await axios.post('/api/deposit/get-deposit-details', {
        accountNumber,
        accountType
      });
  ...
  async function getAccountDetails(event) {
    ...
    setFormData(prev => ({ ...prev, account: accountNumber }));
    // fetchAccountDetails(accountNumber, formData.accountType);
  }

  async function handelAcountSubmit(event) {
    event.preventDefault();
    fetchAccountDetails(formData.account, formData.accountType);
  }
                    ...
                    <Row className="d-flex flex-wrap">
                      ...
                      <Col md="3">
                        <FormGroup>
                          ...
                          <form onSubmit={handelAcountSubmit}>
