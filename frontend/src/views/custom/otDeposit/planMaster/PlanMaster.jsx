...
const PlanMaster = () => {
    ...
    useEffect(() => {
        if(!fetched){
            axios.get("/api/loan/get-plans/deposit")
                .then(function (res) {
                    if(res.data.success){
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.post(
                "/api/loan/plan-creation/deposit",
                { ...formData, id: formData.schemeCode }
            );

            if (response.data.success) {
                setSchemes(prev => {
                    const others = prev.filter(s => s.schemeCode !== formData.schemeCode);
                    return [...others, formData];
                });
