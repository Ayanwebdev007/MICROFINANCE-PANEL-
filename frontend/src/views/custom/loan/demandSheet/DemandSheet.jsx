...
const LoanDemandSheetPage = () => {
    ...
    try {
      ...
      if (res.data.success && Array.isArray(res.data.users)) {
        const employees = [
          ...
          ...res.data.users
            .filter(emp => emp.id) // Firebase UID is in `id`
            .map((emp) => ({
              value: emp.id,
              label: `${emp.name || 'N/A'}${emp.phone ? ` (${emp.phone})` : ''}`,
            })),
          { value: "N/A", label: "Unassigned" },
        ];
        setEmployeeDropDown(employees);
      } else {
        notify(res.data.error || "Failed to fetch employees", "warning");
      }
    } catch (err) {
      console.error("Error fetching employees:", err);
      notify("Failed to load employee list: " + (err.response?.data?.error || err.message), "danger");
    }
  };
  ...
  const fetchDemandSheet = async () => {
    ...
    try {
      const params = {};
      if (selectedLoanType) params.loanType = selectedLoanType;
      if (selectedEmployee) params.employeeId = selectedEmployee;

      const response = await axios.get(`/api/reports/loan/demand-sheet/${selectedDate}`, {
        withCredentials: true,
        params,
      });

      if (response.data.success && Array.isArray(response.data.demand)) {
        const normalizedData = response.data.demand.map((record) => ({
          ...record,
          loanAccount: record.loanAccount || "N/A",
          currentDemand: record.demandCurrent,
        }));
        setRowData(normalizedData);
        notify(`${response.data.totalRecords} EMI demand record(s) loaded.`, "success");
      } else {
        setRowData([]);
        notify(response.data.error || "No EMI demands found for this date.", "warning");
      }
    } catch (err) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const url = `/api/reports/loan/demand-sheet-pdf/${selectedDate}?${params.toString()}`;
      ...
      link.download = `Demand_Sheet_${selectedDate}.pdf`;
      link.click();
  ...
  const handleFetch = useCallback(() => {
    fetchDemandSheet();
  }, [selectedDate, selectedLoanType, selectedEmployee]);
  ...
  const onBtExport = useCallback(() => {
    ...
    gridRef.current.api.exportDataAsCsv({
      ...
      processCellCallback: (params) => {
        ...
        if (["emiAmount", "principal", "interest", "totalDemand"].includes(colId)) {
  ...
  const colDefs = [
        ...
        return (
          <Button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              navigate(`/Active-Loan-Account/Details/${loanAccount}`, {
                state: params.data,
              });
            }}
          >
    ...
    { field: "clientName", headerName: "CLIENT NAME", flex: 1.8 },
            ...
            <Card>
              <CardHeader className="d-flex justify-content-between align-items-center flex-wrap text-white py-3 px-4 bg-light">
                <CardTitle tag="h4" className="mb-2 mb-md-0">
                  ...
                  {selectedEmployee
                    ? ` | Employee: ${employeeDropDown.find((e) => e.value === selectedEmployee)?.label || selectedEmployee}`
                    : ""}
                </CardTitle>
              </CardHeader>
                ...
                {loading ? (
                  ...
                  <div className="text-center my-5">
                    <p className="text-muted">
                      {selectedDate
                        ? "No EMI demands found for this selection."
                        : "Please select a date and click 'Generate Sheet'."}
                    </p>
...
export default LoanDemandSheetPage;